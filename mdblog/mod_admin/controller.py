...
def add_article():
	...
	if add_form.validate():
		...
		return redirect(url_for("blog.view_articles_page"))
	else:
		for error in add_form.errors:
			flash("{} is required".format(error), "alert-danger")
		return render_template("mod_admin/article_editor.html", form=add_form)
...
def view_article_editor(art_id):
	...
	if article:
		...
		return render_template("mod_admin/article_editor.html", form=form, article=article)
...
def edit_article(art_id):
	...
	if article:
		...
		if edit_form.validate():
			...
			return redirect(url_for("blog.view_article_page", art_id=art_id))
		else:
...
def view_add_article():
	if "logged" not in session:
		return redirect(url_for("admin.view_login"))
	form = ArticleForm()
	return render_template("mod_admin/article_editor.html", form=form)
...
def login_user():
	...
	if login_form.validate():
		...
		if user and user.check_password(login_form.password.data):
			...
			return redirect(url_for("admin.view_admin_page"))
		else:
...
def logout_user():
	...
	return redirect(url_for("main.view_velcome_page"))
